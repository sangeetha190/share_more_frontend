  ...
  const onSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      const response = await axios.get(
        `/blood_donor_appointment/${values.unique}`
      );
      if (response.data) {
  ...
  const handleDonatedClick = async (values, setSubmitting) => {
    ...
    try {
      const response = await axios.put(
        `/blood_donor_appointment/update/${values.unique}`
      );
                        ...
                        <Form>
                          <div className="row">
                            ...
                            {getData && getData !== "no-data" && (
                              <div className="col-md-6">
                                <h5 className="mb-2">
                                  Name: {getData?.donor_id?.donorId?.name}
                                </h5>
                                <h5 className="mb-0">
                                  Mobile no:{" "}
                                  {getData?.donor_id?.donorId?.contactNumber}
                                </h5>
                                <button
                                  type="button"
