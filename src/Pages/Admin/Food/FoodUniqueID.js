...
const FoodUniqueID = () => {
  ...
  const onSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      const response = await axios.get(`/share_food/${values.unique}`);
      if (response.data) {
  ...
  const handleDonatedClick = async (values, setSubmitting) => {
    ...
    try {
      ...
      const response = await axios.put(`/share_food/update/${values.unique}`);
                          ...
                          <div className="row">
                            ...
                            {getData && getData !== "no-data" && (
                              <div className="col-md-6">
                                ...
                                <h6 className="mb-2">
                                  Charity Name: {getData.charityId?.name}
                                </h6>
                                <h6 className="mb-0">
                                  Charity Place: {getData.charityId?.district}
                                </h6>
                                {/* charityId */}
                                <button
                                  type="button"
